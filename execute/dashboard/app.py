...
# Dual-strategy credential loading: Streamlit Secrets (Cloud) or Environment Variables (Local)
...
if not NEO4J_PASSWORD:
    st.error("Neo4j password not found. Configure .env for local run or secrets.toml for Streamlit Cloud.")
    st.stop()
...
def run_cypher(query: str, params: dict = None) -> pd.DataFrame:
...
if page == "Overview":
    ...
    try:
        ...
        st.warning(f"Unable to fetch overview statistics. Ensure graph loading pipeline has been executed. ({err})")
...
# ---------------------------------------------------------
    ...
    if not df_comm.empty:
        ...
        st.info("No community detection data found. Run GDS analytics pipeline (`run_analytics.py`) first.")